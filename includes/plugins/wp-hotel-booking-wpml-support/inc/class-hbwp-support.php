...
class HBWPML_Support {
    ...
    public function init() {
        $sessions = WP_Hotel_Booking::instance()->cart->sessions;
        if ( !$sessions->session ) {
            return;
        }
        ...
        foreach ( $sessions->session as $cart_id => $param ) {
            ...
            WP_Hotel_Booking::instance()->cart->remove_cart_item( $cart_id );
            WP_Hotel_Booking::instance()->cart->add_to_cart( $param['product_id'], $param, $qty );
        }
    ...
    public function wpml_switcher_language( $lang ) {
        ...
        if ( !WP_Hotel_Booking::instance()->cart->is_empty() ) {
            var_dump( WP_Hotel_Booking::instance()->cart->session );
            die();
        }
