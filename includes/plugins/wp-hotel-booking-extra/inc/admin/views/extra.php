...
<p class="description"><?php _e( 'Adding room\'s services packages with detail price for every service', 'wp-hb-extra' ); ?></p>
<form action="" class="tp_extra_form_field_settings" method="POST">
    <div id="tp_extra_form">
        <div class="tp_extra_form_head">
            <h3><?php _e( 'Extra Options', 'wp-hb-extra' ); ?></h3>
        </div>
        <?php if( $extras ): ?>
            <?php foreach( $extras as $k => $post ): ?>
                ...
                <div class="tp_extra_form_fields">
                    ...
                    <div class="price">
                        ...
                        <span>/</span>
                        <input type="text" name="<?php echo esc_attr( $field_name ); ?>[<?php echo esc_attr( $post->ID ); ?>][respondent_name]" value="<?php echo esc_attr( get_post_meta( $post->ID, 'tp_hb_extra_room_respondent_name', true ) ); ?>" placeholder="<?php esc_attr_e( 'Package', 'wp-hb-extra' ) ?>"/>
                    </div>
                    <div class="type">
        ...
        <?php else: ?>
            ...
            <div class="tp_extra_form_fields">
                ...
                <div class="price">
                    ...
                    <span>/</span>
                    <input type="text" name="<?php echo esc_attr( $field_name ); ?>[0][respondent_name]" value="" placeholder="<?php esc_attr_e( 'Package', 'wp-hb-extra' ) ?>"/>
                </div>
                <div class="type">
                    <h4><?php _e( 'Price Type', 'wp-hb-extra' ); ?></h4>
        ...
        <div class="tp_extra_form_foot">
            <button type="submit" class="button button-primary"><?php _e( 'Save Extra', 'wp-hb-extra' ); ?></button>
            <a class="button tp_extra_add_item"><?php _e( 'Add another item', 'wp-hb-extra' ); ?></a>
        </div>
    </div>
</form>
...
<script type="text/html" id="tmpl-tp-hb-extra-room">
    <div class="tp_extra_form_fields">
        ...
        <div class="desc">
            <h4><?php _e( 'Description', 'wp-hb-extra' ); ?></h4>
            <textarea name="<?php echo esc_attr( $field_name ) ?>[{{ data.id }}][desc]" placeholder="<?php esc_attr_e( 'Enter description here', 'wp-hb-extra' ) ?>"></textarea>
        </div>
        <div class="price">
            <h4><?php _e( 'Price', 'wp-hb-extra' ); ?></h4>
            <input type="number" step="any" name="<?php echo esc_attr( $field_name ); ?>[{{ data.id }}][price]" value="" placeholder="<?php echo esc_attr( '10.5' ) ?>"/>
            <span>/</span>
            <input type="text" name="<?php echo esc_attr( $field_name ); ?>[{{ data.id }}][respondent_name]" value="" placeholder="<?php esc_attr_e( 'Package', 'wp-hb-extra' ) ?>"/>
        </div>
        <div class="type">
            <h4><?php _e( 'Price Type', 'wp-hb-extra' ); ?></h4>
