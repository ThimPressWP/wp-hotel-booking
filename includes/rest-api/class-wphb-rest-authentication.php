<?php
/**
 * Class WPHB_REST_Authentication
 *
 * @since 3.3.0
 */
class WPHB_REST_Authentication {
	...
	public function is_rest_api_request() {
		...
		return apply_filters( 'wphb/is-rest-api-request', false !== strpos( $request_uri, $rest_prefix . 'wphb/' ) );
	}
	...
	public function perform_oauth_authentication() {
		...
		if ( empty( $this->user ) ) {
			$this->set_error( new WP_Error( 'wphb_rest_authentication_error', __( 'Consumer key is invalid.', 'wp-hotel-booking' ), array( 'status' => 401 ) ) );
	...
	public static function check_post_permissions( $post_type, $permission = 'read', $user_id = 0 ) {
		...
		return apply_filters( 'wphb/rest/check-permission', $user_permission, $permission, $user_id, $post_type );
	}
...
return new WPHB_REST_Authentication();
