 ...
 * @Last Modified time: 2016-03-30 15:08:02
 */
	...
	public static function render_fields( $fields = array() ) {
		...
		foreach ( $fields as $k => $field ) {
			...
			switch ( $field['type'] ) {
				...
				case 'email':
				case 'password':
					$value = hb_settings()->get( $field['id'], isset( $field['default'] ) ? $field['default'] : '' );
						?>
							<tr valign="top">
								<th scope="row">
									<?php if ( isset( $field['title'] ) ) : ?>
				...
				case 'checkbox':
						$val = hb_settings()->get( $field['id'], isset( $field['default'] ) ? $field['default'] : '' );
						?>
							<tr valign="top"<?php echo isset( $field['trclass'] ) ? ' class="'.implode( '', $field['trclass'] ).'"' : '' ?>>
								...
								<td class="hb-form-field hb-form-field-<?php echo esc_attr( $field['type'] ) ?>">
									...
									<input type="checkbox" name="<?php echo isset( $field['id'] ) ? esc_attr( $field['id'] ) : '' ?>" <?php echo $custom_attr ?><?php checked( $val, $field['default'] ); ?>/>
								</td>
							</tr>
						<?php
					break;
								...
								<td class="hb-form-field hb-form-field-<?php echo esc_attr( $field['type'] ) ?>">
									<?php if ( isset( $field['options'] ) ) : ?>
										<?php foreach ( $field['options'] as $val => $text ) : ?>
											...
											<label>
												<input type="radio" name="<?php echo isset( $field['id'] ) ? esc_attr( $field['id'] ) : '' ?>" />
												<?php echo esc_html( $text ) ?>
											</label>
