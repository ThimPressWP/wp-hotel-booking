...
<?php $currency_symbol = wphb_get_currency_symbol( $book->currency ); ?>

<?php if ( $book->get_status() !== 'completed' ): ?>
	<?php do_action( 'hotel_booking_order_pay_before' ); ?>
    <h3><?php printf( __( 'Booking ID: %s', 'wp-hotel-booking' ), wphb_format_order_number( $booking_id ) ) ?></h3>
    <p>
    ...
    <p>
		...
		<?php printf( '%s', wphb_format_price( hb_booking_total( $booking_id ), $currency_symbol ) ) ?>
    </p>
    <p>
        <strong><?php _e( 'Advance Payment: ', 'wp-hotel-booking' ) ?></strong>
		<?php printf( '%s', wphb_format_price( $book->advance_payment, $currency_symbol ) ) ?>
    </p>
