        ...
        <form name="hb-payment-form" id="hb-payment-form" method="post" action="<?php echo isset( $search_page ) ? $search_page : ''; ?>">
            ...
            <table class="hb_table">
				...
				<?php if ( $tax = hb_get_tax_settings() ) { ?>
                    <tr class="hb_advance_tax">
                        <td colspan="8">
							...
							<?php } ?>
                            <span class="hb-align-right"><?php echo apply_filters( 'hotel_booking_cart_tax_display', wphb_format_price( $cart->total - $cart->sub_total ) ); // abs( $tax * 100 ) . '%' ?></span>
                        </td>
                    </tr>
				<?php } ?>
				...
				<?php if ( $advance_payment = $cart->advance_payment ) { ?>
					...
					<?php if ( wphb_get_advance_payment() < 100 ) { ?>
			...
			<?php if ( !is_user_logged_in() && !wphb_settings()->get( 'guest_checkout' ) && get_option( 'users_can_register' ) ) : ?>

				<?php printf( __( 'You have to <strong><a href="%s">login</a></strong> or <strong><a href="%s">register</a></strong> to checkout.', 'wp-hotel-booking' ), wp_login_url( wphb_get_checkout_url() ), wp_registration_url() ) ?>

			<?php else : ?>

				<?php wphb_get_template( 'checkout/customer.php', array( 'customer' => $customer ) ); ?>
				<?php wphb_get_template( 'checkout/payment-method.php', array( 'customer' => $customer ) ); ?>
				<?php wphb_get_template( 'checkout/addition-information.php' ); ?>
				...
				<?php if ( $tos_page_id = wphb_get_page_id( 'terms' ) ) { ?>
                    <p>
                        <label>
